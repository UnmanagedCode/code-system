...
- **reap** — a kind's relay of the kill into the far side, for children that are not the launcher's OS descendants (`Transport.reap`). For `docker`, a `/proc` scan for the exec's `CC_EXEC_TOKEN`.
